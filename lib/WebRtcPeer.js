...
function trackStop(track) {
  console.log("WRP : trackStop()");
  track.stop && track.stop()
}

function streamStop(stream) {
  console.log("WRP : streamStop()");
...
function bufferizeCandidates(pc, onerror) {
  console.log("WRP : bufferizeCandidates()");
  var candidatesQueue = []
...
function WebRtcPeer(mode, options, callback) {
  ...
  // Init PeerConnection
  if (!pc) {
    console.log("WRP : init peer connection : !pc");
  ...
  function setRemoteVideo() {
    console.log("WRP : setRemoteVideo")
    if (remoteVideo) {
      console.log("WRP : remoteVideo : ", remoteVideo);
      console.log("WRP : remoteVideo streams : ", pc.getRemoteStreams());
      var stream = pc.getRemoteStreams()[0]
      var url = stream ? URL.createObjectURL(stream) : ''
      console.log("WRP : remoteVideo url : ", url);
      ...
      //remoteVideo.pause()
      //remoteVideo.src = url
      console.log("WRP : remoteVideo src : ", url);
      //remoteVideo.load()

      logger.info('WRP : Remote URL:', url)
  ...
  this.showLocalVideo = function () {
    console.log("WRP : showLocalVideo");
    localVideo.src = URL.createObjectURL(videoStream)
    console.log("WRP : localVideo src : ", localVideo.src);
    localVideo.muted = true
  }

  this.getLocalVideoSrc = function () {
    console.log("WRP : showLocalVideo");
    var localVideoSrc = URL.createObjectURL(videoStream)
    console.log("WRP : localVideoSrc : ", localVideo.src);
    localVideo.muted = true
    return localVideoSrc;
  }

  this.send = function (data) {
    console.log("WRP : send : data ", data);
    console.log("WRP : send : dataChannel ", dataChannel);
    if (dataChannel && dataChannel.readyState === 'open') {
      console.log("WRP : send : dataChannel.ready state === 'open'");
  ...
  function mangleSdpToAddSimulcast(answer) {
    console.log("WRP : mangleSdpToAddSimulcast");
    if (simulcast) {
      if (browser.name === 'Chrome' || browser.name === 'Chromium') {
  ...
  function start() {
    ...
    if (videoStream && localVideo) {
      console.log("WRP : videoStream && localVideo");
      var localVideoSrc = self.getLocalVideoSrc();
      // NOTE : JAB : 11/18/16
      // Uncommenting the code below will display the local video stream, however, we are
      // currently using the Pluot function setupLocalVideo() in call-machine.js
      // to display local video.
      //self.showLocalVideo();
      console.log("WRP : localVideoSrc from start() : ", localVideoSrc);
    ...
    if (videoStream) {
      console.log("WRP : videoStream");
      pc.addStream(videoStream)
    }

    if (audioStream) {
      console.log("WRP : audioStream");
  ...
  if (mode !== 'recvonly' && !videoStream && !audioStream) {
    console.log("WRP : !recvonly && !videoStream && !audioStream");
    function getMedia(constraints) {
      ...
      navigator.mediaDevices.getUserMedia(constraints).then(function (stream) {
        videoStream = stream;
        console.log("WRP : videoStream : ", stream);
        start();
      }).catch(callback);
    }
    if (sendSource === 'webcam') {
      console.log("WRP : sendSource === webcam");
      console.log("WRP : mediaConstraints : ", mediaConstraints);
...
function createEnableDescriptor(type) {
  console.log("WRP : createEnableDescriptor");
...
WebRtcPeer.prototype.dispose = function () {
  console.log("WRP : disposing WebRtcPeer");
...
function WebRtcPeerRecvonly(options, callback) {
  console.log("WRP : WebRtcPeerRecvonly");
...
function WebRtcPeerSendonly(options, callback) {
  console.log("WRP : WebRtcPeerSendonly");
...
function WebRtcPeerSendrecv(options, callback) {
  console.log("WRP : WebRtcPeerSendrecv");
