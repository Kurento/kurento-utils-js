...
function WebRtcPeer(mode, options, callback) {
  ...
  var videoDisabled = options.videoDisabled || false;
...
*/
  this.replaceTrack = function (stream, streamType = "video") {
    let track;
    if (streamType == "video") {
    ...
    let sender = pc
      .getSenders()
      .find((s) => s.track && s.track.kind === streamType);
    if (sender) {
      sender.replaceTrack(track);
    }
  };
  ...
  this.processAnswer = function (sdpAnswer, callback) {
    ...
    pc.setRemoteDescription(answer)
      ...
      .catch((error) =>
        logger.debug(
          "error while setting remote description : " + error.message
        )
      );
  };
