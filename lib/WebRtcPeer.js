...
function WebRtcPeer(mode, options, callback) {
  ...
  const videoStream = options.videoStream
  ...
  const {dataChannelConfig} = options
  ...
  const {oncandidategatheringdone} = options
  if (oncandidategatheringdone) {
    this.on('candidategatheringdone', oncandidategatheringdone)
  }

  const {multistream, simulcast} = options
  ...
  this.generateOffer = function (callback) {
    ...
    if (typeof AdapterJS !== 'undefined' && AdapterJS
      ...
      function setLocalDescriptionOnSuccess() {
        sleep(1000);
      ...
      function createOfferOnSuccess(offer) {
